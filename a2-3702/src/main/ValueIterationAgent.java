...
public class ValueIterationAgent implements OrderingAgent {
	...
	private void recursiveGeneration (int generateType, int maxSum, int remaining, int currentIndex, Map<Integer, Integer> currentMap, int maxVal) {
		...
		System.out.println("recursiveGeneration: remaining: " + remaining);
		System.out.println("recursiveGeneration: current index: " + currentIndex);
		System.out.println("recursiveGeneration: maximum index: " + (this.fridge.getMaxTypes() - 1));
		
		if (remaining < 0 || currentIndex > (this.fridge.getMaxTypes() - 1)) {
			System.out.println("Base Case: Done at index " + currentIndex + " with " + remaining + " left");
